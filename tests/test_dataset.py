...
def test_load_dns_trajectory_samples_fixed_sensors(tmp_path) -> None:
    ...
    assert trajectory.sensor_values.shape == (6, 10)
    assert trajectory.physics_grid_shape == (4, 4)
    assert trajectory.physics_coords.shape == (16, 2)
    assert trajectory.physics_values.shape == (6, 16)
...
def test_build_dataset_shapes_and_time_coords(tmp_path) -> None:
    ...
    assert train_branch.shape == (3, 13)
    assert dataset.train_y.shape == (3, 12)
    assert val_branch.shape == (1, 13)
    assert dataset.val_y.shape == (1, 12)
    assert test_branch.shape == (2, 13)
    assert dataset.test_y.shape == (2, 12)
    assert train_trunk.shape == (12, 3)
    assert val_trunk.shape == (12, 3)
    assert test_trunk.shape == (12, 3)
...
def test_physics_informed_data_batch_contract(tmp_path) -> None:
    ...
    assert x_batch[0].shape == (2, 9)
    assert x_batch[1].shape == (8, 3)
    assert y_batch.shape == (2, 8)
...
def test_physics_branch_batch_size_only_applies_to_physics_loss(tmp_path) -> None:
    ...
    def fake_physics_loss(model, branch_inputs, physics_coords_xy, delta_t_tensor, nu_tensor, forcing_tensor):
    ...
    inputs = (
        torch.randn(n_train, dataset.train_x[0].shape[1]),
...
""".strip()
    ...
    assert args.ic_loss_weight == 15.0
...
def test_create_model_supports_gated_mlp_forward() -> None:
    ...
    branch = torch.randn(2, 51)
    trunk = torch.randn(50, 3)
